...
5. Bonus freestyle programming: as all the variables are coded as "double" type (=continuous) but some of them are binary or catgeorical, how would you change them into the appropriate type?
```{r}
...
SPOILER - code below
- Labels of categorical variables have been lost in the importation, I have no idea how to fix this, do you?
```{r}
# Import the dataset
trin <- read.dta("./trinmlsh.dta")
...
trin <- trin %>% mutate(smokenum = as.ordered(smokenum))

# Unify non-smokers + ex-smokers vs current smokers
trin <- trin %>%
  mutate(smokstatus = as.ordered(
    case_when(
      smokenum == 0 ~ "0",
      smokenum == 1 ~ "0",
      smokenum == 2 ~ "1",
...
# Label the values in this new variable
trin$smokstatus<-
 factor(trin$smokstatus,
   levels = c(0,1),
   labels = c("non-smoker", "active smoker"))
